...
def mouse_lines(events: list[TraceEvent]) -> list[str]:
    ...
    lines = [
        ...
        "# Optional delay modifiers also work here, for example: Move: 1.2 s + 0.5 s -k delta (10, -4)",
        "",
    ]
    ...
    for event in events:
        ...
        if event.event_type == EV_MOUSE_WHEEL:
            lines.append(f"Wheel: {format_time_ns(event.t_ns)} delta {event.wheel}")
            continue
        ...
        hold_ns = max(0, event.t_ns - start_event.t_ns)
        lines.append(f"{canonical}: {format_time_ns(start_event.t_ns)} ({format_time_ns(hold_ns)})")
    ...
    for label in ["Left Click", "Right Click", "Middle Click", "X1 Click", "X2 Click"]:
        ...
        if start_event is None:
            continue
        lines.append(f"{canonical}: {format_time_ns(start_event.t_ns)} ({format_time_ns(0)})")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Decode relative trace data into readable keyboard and mouse files.")
    parser.add_argument("--input", type=Path, default=default_input_path(), help="Source generated .c or .bin file")
    parser.add_argument("--keyboard-out", type=Path, default=KEYBOARD_TEXT, help="Keyboard readable output path")
    parser.add_argument("--mouse-out", type=Path, default=MOUSE_TEXT, help="Relative mouse readable output path")
