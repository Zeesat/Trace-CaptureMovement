        ...
        try:
            ...
            if normalized_label == "move":
                ...
                entries.append(
                    build_entry(
                        ...
                        affected_categories=affected_categories,
                        sort_order=line_number,
                        events=[EventSpec(0, EV_MOUSE_MOVE, 0, 0, x, y, 0)],
                    )
                )
            ...
            if normalized_label == "wheel":
                ...
                entries.append(
                    build_entry(
                        ...
                        affected_categories=affected_categories,
                        sort_order=line_number,
                        events=[EventSpec(0, EV_MOUSE_WHEEL, 0, 0, x, y, delta)],
                    )
                )
                if start_shift_ns and not affected_categories:
                    warnings.append(f"{path.name}:{line_number}: shift ignored because both -m and -k remove all targets")
            ...
            entries.append(
                build_entry(
                    ...
                    sort_order=line_number,
                    events=[
                        EventSpec(0, down_type, 0, 0, x, y, 0),
                        EventSpec(max(0, hold_ns), up_type, 0, 0, x, y, 0),
                    ],
                )
            )
